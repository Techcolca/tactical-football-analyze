...
function detectLanguage(text: string): string {
    ...
    const countExactWords = (words: string[], text: string): number => {
    ...
    if (frenchScore > spanishScore) return 'french';
    return 'english';
}
...
function getExamplePromptByLanguage(language: string): string {
    switch (language) {
        case 'spanish':
...
export class GroqClient {
    ...
    private getSystemPromptByLanguage(language: string): string {
        switch (language) {
            case 'es':
    ...
    private getExamplePromptByLanguage(language: string): string {
        switch (language) {
            case 'es':
    ...
    private getExampleResponseByLanguage(language: string): string {
        switch (language) {
            case 'es':
...
- 4-3-3 counter-attack`;
    ...
    async generateTacticalAnalysis(prompt: string, language: string = 'en'): Promise<string> {
        try {
            ...
            const messages = [
                { 
                  role: 'system', 
                  content: `${systemPrompt}\n\nIMPORTANT: You MUST respond ONLY in ${
                    language === 'es' ? 'SPANISH' : 
                    language === 'fr' ? 'FRENCH' : 
                    'ENGLISH'
            ...
            if (!response.data.choices || response.data.choices.length === 0) {
                throw new Error(
                  language === 'es' ? 'No se recibió respuesta del modelo' :
            ...
            throw new Error(
              language === 'es' ? 'Error al generar el análisis: ' + (error.message || 'Error desconocido') :
